...
open class CustomFetchNotiManager(context: Context) : FetchNotificationManager {
    ...
    var progressReportingIntervalInMillis: Long = 0L
...
class CustomFetchNotificationManager(context: Context) : FetchNotificationManager {
    ...
    override fun updateNotification(notificationBuilder: NotificationCompat.Builder,
        ...
        val currentProgress = "%.2f".format(FetchingUtils.getProgress(downloadNotification.downloaded, downloadNotification.total))
        val speed = FetchingUtils.getDownloadSpeedString(downloadNotification.downloadedBytesPerSecond)
        notificationBuilder.setPriority(NotificationCompat.PRIORITY_DEFAULT)
                .setSmallIcon(smallIcon)
                .setContentTitle(downloadNotification.title)
