...
public class ShelfView extends GridView implements AdapterView.OnItemClickListener {
    ...
    public void loadData(final List<BookModel> bookModel) {
        new Handler().postDelayed(() -> processData(bookModel), 300);
    }
    ...
    private void processData(final List<BookModel> bookModel) {
        ...
        for (int i = 0; i < bookModel.size(); i++) {
            ...
            int count = bookModel.get(i).getBadgeCount();
            BookSource bookSource = bookModel.get(i).getBookSource();
            if ((i % mNumberOfTilesPerRow) == 0) {
                mShelfModels.add(new ShelfModel(bookCoverSource, bookId, bookTitle, true, ShelfType.START, bookSource, count));
            } else if ((i % mNumberOfTilesPerRow) == (mNumberOfTilesPerRow - 1)) {
                mShelfModels.add(new ShelfModel(bookCoverSource, bookId, bookTitle, true, ShelfType.END, bookSource, count));
            } else {
                mShelfModels.add(new ShelfModel(bookCoverSource, bookId, bookTitle, true, ShelfType.NONE, bookSource, count));
            }
        }
